...
const NetworkStatus = props => {
  return (
    <div>
      ...
      <span title="If WRITE access is not ticked, make sure a valid network is configured and that network is also linked to either your Scatter or Sqrl identity.">
        Write: {status(props.writerEnabled, props.writerLoading)}{' '}
      </span>
      <span title="If ACCOUNT is not ticked make sure to use the 'Select account' menu above and select either a Scatter or Sqrl identity and account to perform actions under.">
